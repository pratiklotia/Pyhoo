...
parser.add_argument("-i","--iteration", help="(Optional) Number of emails to parse in reverse order; default is 10",\
                    type=int)
...
unsubscribe_words = ['Unsubscribe', 'unsubscribe', 'Stop', 'stop', 'Click', 'click', 'update', 'Update', 'Subscrib',\
                     'subscrib', 'Choose', 'choose'] # 'Click here', 'subscribe' or 'subscription'
for i in range(int(iteration)):
    ...
    # Make the tests...
        ...
        for i in unsubscribe_words:
            if driver.find_element_by_partial_link_text(i).send_keys(Keys.COMMAND + Keys.RETURN):
                action = "function_for_unsubscribe?" # most pages get unsubscribed when you click on it - some ask for \
                # confirmation though
